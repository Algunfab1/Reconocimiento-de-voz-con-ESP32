...
{
    Serial.printf("estado de conexion con el servidor: ");
...
Intent WitAiChunkedUploader::getResults()
{
    ...
    if (status == 200)
    {
        ...
        //filter["entities"]["wit$message_body:message_body"][0]["value"] = true;
        //filter["entities"]["wit$message_body:message_body"][0]["confidence"] = true;
        filter["entities"]["wit$contact:contact"][0]["value"] = true;
        filter["entities"]["wit$contact:contact"][0]["confidence"] = true;
        filter["text"] = true;
        ...
        const char *intent_name = doc["intents"][0]["name"]; // nombre de la intencion
        float intent_confidence = doc["intents"][0]["confidence"]; // grado de confianza
        const char *device_name = doc["entities"]["wit$contact:contact"][0]["value"]; // nombre del dispositivo
        float device_confidence = doc["entities"]["wit$contact:contact"][0]["confidence"]; // grado de confianza del dispositivo
        const char *trait_value = doc["traits"]["wit$on_off"][0]["value"]; // valor del rasgo
        float trait_confidence = doc["traits"]["wit$on_off"][0]["confidence"]; // grado de confianza del rasgpo
       // imprimo si se detecto un sms
